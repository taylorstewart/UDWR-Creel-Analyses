...
ggplot(dat_sum, aes(x = month, y = lod)) +
  ...
  scale_x_continuous(limits = c(0.5, 12.5), breaks = seq(1, 12, 1), expand = c(0, 0.1)) +
  scale_y_continuous(limits = c(0, 16), breaks = seq(0, 16, 2), expand = c(0, 0.05)) +
...
cal_wd_one <- cal %>% filter(dow == "wd") %>% 
  ...
  mutate(shift = sample(rep(c("AM", "PM"), each = ceiling(n()/2)), size = n(), replace = FALSE))
